...
for _ in range(t_step):
    if np.random.uniform(0, 1) > trade_occur_rate:
        x = F @ x + u + np.random.multivariate_normal(np.zeros(2), Q).reshape((-1, 1))  # x: shape (2, 1)
        x_hist.append(x); z_hist.append(None); order_type.append(0)
    else:
        x = F @ x + u + np.random.multivariate_normal(np.zeros(2), Q).reshape((-1, 1))  # x: shape (2, 1)
        ...
        x_hist.append(x); z_hist.append(z); order_type.append(ot)

x_hist = np.concatenate(x_hist, axis=1)  # x_hist: shape (2, T)
order_type = np.array(order_type)  # order_type: shape (T,)

# Kalman filter estimates with known parameters (F, Q, R)
...
for t_idx in range(t_step):
    ...
    if t_idx == 0 or order_type[t_idx - 1] != 0:
        x_prior_est = F @ x_posterior_est + u  # x_prior_est: shape (2, 1)
        p_prior_est = F @ p_posterior_est @ (F.T) + Q  # p_prior_est: shape (2, 2)
    else:
        x_prior_est = F @ x_prior_est + u  # x_prior_est: shape (2, 1)
    ...
    if order_type[t_idx] == 0:
        ...
        if nonlinear_type == "abs":
            H = np.array([1, 0.5 * order_type[t_idx] * np.sign(x_prior_est[1, 0])]).reshape((1, -1))  # H: shape (1, 2)
            innovation = z_hist[t_idx] - (coef @ np.array([x_prior_est[0, 0], np.abs(x_prior_est[1, 0])]).reshape((-1, 1))) # innovation: shape (1, 1)
        if nonlinear_type == "square":
            H = np.array([1, order_type[t_idx] * x_prior_est[1, 0]]).reshape((1, -1))  # H: shape (1, 2)
...
if nonlinear_type == "abs":
    ...
    max_ = []; min_ = []
    for t_idx in range(t_step):
        max_.append(max(np.abs(x_prior_est_hist[1, t_idx] - np.sqrt(p_prior_est_hist[1, t_idx])), np.abs(x_prior_est_hist[1, t_idx] + np.sqrt(p_prior_est_hist[1, t_idx]))))
        if x_prior_est_hist[1, t_idx] - np.sqrt(p_prior_est_hist[1, t_idx]) < 0:
            min_.append(0)
        else:
            min_.append(np.abs(x_prior_est_hist[1, t_idx] - np.sqrt(p_prior_est_hist[1, t_idx])))
    plt.fill_between(t_hist, min_, max_, alpha=0.3, color="C1")
...
#%% Parameter estimation by MLE with automatic differentiation
...
plt.subplot(4, 1, 3)
print(train_log[-1][2][0], np.diag(Q)[0])
