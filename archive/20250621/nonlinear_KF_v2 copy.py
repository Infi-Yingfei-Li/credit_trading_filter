...
for _ in range(t_step):
    if np.random.uniform(0, 1) > trade_occur_rate:
        x = F.dot(x) + np.random.multivariate_normal(np.zeros(2), Q).reshape((-1, 1))  # x: shape (2, 1)
        ...
        H = np.array([[1, 0.5*ot*np.sign(x[1, 0])]]).reshape((1, -1))  # H: shape (1, 2)
        z = coef @ np.abs(x) + np.random.normal(0, R).reshape((1, 1))  # z: shape (1, 1)

        #H = np.array([[1, 0.5*ot*np.sign(x[1, 0])]]).reshape((1, -1))  # H: shape (1, 2)
        #z = H.dot(x) + np.random.normal(0, R).reshape((1, 1))  # z: shape (1, 1)

        #if np.random.uniform(0, 1) <= ask_prop:
        #if ot == 1:
...
for t_idx in range(t_step):
    ...
    if t_idx == 0 or order_type[t_idx - 1] != 0:
        x_prior_est = F.dot(x_posterior_est)  # x_prior_est: shape (2, 1)
        p_prior_est = F.dot(p_posterior_est).dot(F.T) + Q  # p_prior_est: shape (2, 2)
    else:
        x_prior_est = F.dot(x_prior_est)  # x_prior_est: shape (2, 1)
        p_prior_est = F.dot(p_prior_est).dot(F.T) + Q  # p_prior_est: shape (2, 2)
    ...
    if order_type[t_idx] == 0:
        ...
        innovation_cov = H.dot(p_prior_est).dot(H.T) + R  # innovation_cov: shape (1, 1)
