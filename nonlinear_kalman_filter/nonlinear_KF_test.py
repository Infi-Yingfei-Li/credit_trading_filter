#%%
import os, datetime, tqdm, collections, time, pickle
...
def kf_core():
    ...
    for t_idx in range(t_step):
        ...
        if order_type[t_idx] == 0:
            ...
            if nonlinear_type == "square":
                ...
                innovation = z_hist[t_idx] - (coef @ np.array([[x_prior_est[0, 0], x_prior_est[1, 0]**2]]))  # innovation: shape (1, 1)
            H = H_est
    ...
    def _log_likelihood_torch(F_diag_est, Q_diag_est, R_est):
        ...
        for t_idx in range(t_step):
            ...
            if order_type[t_idx] != 0:
                ...
                # estimated H
                H = torch.cat([torch.ones(1), 0.5 * order_type[t_idx] * torch.sign(x_prior_est[1])]).reshape((1, 2)).to(torch.float64)  # shape (1, 2)
    ...
    return {"log_likelihood": (log_likelihood, train_log[-1][0]),
            "F_est": (np.diag(F), train_log[-1][1]),
